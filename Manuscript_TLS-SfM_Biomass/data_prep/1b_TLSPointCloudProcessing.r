# Title: TLSPointCloudProcessing.R

# Objective:
# This script calculates structural metrics for each 0.5x0.5 voxel plot from TLS point clouds
# this will be used in subsequent analyses 
#
# Authors: Brian Drye, Gina Cova, Deborah Nemens
# Date: January 2024
#
# Project: SERDP-funded 3D fuels project (RC19-1064)
#
# How to use this script:
# First run 1_BiomassDataPrep.R
# Create a list of the 3D Fuels field sites selected for analysis ("list_of_sites")
# Download the site folders for selected sites
# Correct Input Folder filepaths to point to these downloaded folders
# Correct Output Folder filepath to desired output location
...
# Input Folder Path:
base_folder <- "C:/Users/dnemens/Box/UW FLAME Lab/PROJECTS/3D FUELS/"
TLS_pointClouds <-  "/data/tls/plots/clipplot/"
#
# Output folder path:
metrics_file <- paste0(base_folder, "/_Common_files/global_inputs/tls/WholePlotStructuralMetrics.csv")
...
# file path to folders of 0.5 x 0.5 m clipped .laz files (point clouds of clip plots)
list_of_sites <-
  c(
    'aucilla',
    'blackwater',
  ...
  list_of_files <-
    list.files(
      path = paste0(
        ...
        site,
        TLS_pointClouds),
      recursive = T,  full.names = TRUE, pattern = ".laz")
...
for(j in 1:length(list_of_files)) {
  ...
  #Occupied volume = number of voxels in whole area / number of possible voxels (250)
...
V.final = V.Store
#create Site and clip plot names from file names
V.final$Site <- gsub(".*FUELS/", "", V.final$file)
V.final$Site <- gsub("/data.*", "", V.final$Site)
V.final$ClipPlot <-  gsub(".*clipplot/", "", V.final$file)
V.final$ClipPlot  <-  gsub(".laz.*", "", V.final$ClipPlot)
V.final$ClipPlot  <-  gsub(".las.*", "", V.final$ClipPlot)
...
#clean up clip plot naming conventions
V.final$ClipPlot <- gsub("CP_", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub("V", "", V.final$ClipPlot)
V.final$ClipPlot = gsub("las.las", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub("THB_", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub("THA_", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub(" _ Cloud.las", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub("THA_", "", V.final$ClipPlot)
V.final$ClipPlot <- gsub("THB_", "", V.final$ClipPlot)
V.final$ClipPlot <- (gsub("L1_", "", V.final$ClipPlot))
V.final$ClipPlot <- (gsub("L2_", "", V.final$ClipPlot))
V.final$ClipPlot <- (gsub("L3_", "", V.final$ClipPlot))
V.final$ClipPlot <- (gsub("L4_", "", V.final$ClipPlot))
...
#remove _POST from clip plot name
...
V.final[V.final$Visit == "POST"|"Post", "destructiveYN"] = "Y"
#Non-dest/ND plots get N in destructiveYN column
V.final <- V.final %>%
  mutate(destructiveYN = case_when(grepl("NonDest|ND", ClipPlot)~"N")) 
#all other PRE plots are destructive
V.final$destructiveYN[is.na(V.final$destructiveYN)] <- "Y"

#clean up ft stew/sycan clipplot names
V.final$ClipPlot <- V.final$ClipPlot %>% str_replace_all(pattern=c("Pre_NonDest_"= "", "Pre_Dest_" = "", "PostFire_Post_" = "", "_ND" = "", "_PRE" = ""))

#remove unnecessary columns
V.final <- V.final[,3:19]

# write to file
write.csv(V.final, file = metrics_file, row.names = F)