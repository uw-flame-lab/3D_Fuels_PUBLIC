...
#Fix from issues section
#	for column in extra_columns: df[column] = df[column].fillna(-1)
#	replace NaN with -1 df[column] = df[column].replace([np.inf, -np.inf], -1) 
#	replace infinities with -1 las_file[column] =
#	df[column].astype(extended_columns_with_data_types[column])
#Addition to fix error
#for column in extra_columns:
