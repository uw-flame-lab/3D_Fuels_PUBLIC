...
base_folder = "D:/filter_rerun/ALS/"
...
for (site in list_of_sites) {
  ...
  laz_files <- list.files(input_folder,".laz", full.names = T)
  ...
  for ( i in 1:length(laz_files)) {
    ...
    this_name <- gsub("_filtered.laz", "_tree_tops.shp", lazfiles_names[i])
    #this_name <- gsub(".laz", "_tree_tops.shp", lazfiles_names)
    ...
    writeOGR(metrics, (paste0(output_folder, gsub(".laz", "_crown_metrics_poly.shp", lazfiles_names[i]))), drive="ESRI Shapefile", overwrite=TRUE)
