...
# NOTE: run 00_classify_normalize_als.r and 01_canopy_metrics_grid.R first 
# this script will read files from  D:\temp\<site>\outputs\als\01_basics\chm\
# script will create a folders in each site subfolder
# D:\temp\<site>\outputs\als\03_forest_gaps
#
# Required libraries/modules: 
#
#install.packages("pacman")
require(pacman) # load package to manage the R packages
p_load(lidR, terra, sf, RCSF, ForestGapR, raster)
...
for (site in list_of_sites) {
	...
	for ( i in 1:length(chmfiles_names)){
		...
		write.csv(gaps_stats,paste0(gap.dir, gsub("_chm.tiff","_gaps.csv",chmfiles_names[i])), row.names=F, sep=",")
